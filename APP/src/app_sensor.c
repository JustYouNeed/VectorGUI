...
/*
  ...
  *******************************************************************************************************
*/
static xSemaphoreHandle	sensorsDataReady;/*  传感器数据准备好标志量  */
...
*                          sensors_Axis3fLpf                
*
* Description: 传感器数据二阶低通滤波
*             
* Arguments  : 1.data: 低通滤波器数据地址
*							 2.in: 传感器数据存储地址
*
* Reutrn     : None.
*
...
{	
	...
	bsp_mpu_SetFullScaleGyroRange(SENSORS_GYRO_FS_CFG);	// 设置陀螺量程	
	bsp_mpu_SetFullScaleAccelRange(SENSORS_ACCEL_FS_CFG);// 设置加速计量程	
	bsp_mpu_SetAccelDLPF(MPU_ACCEL_DLPF_BW_41);		// 设置加速计数字低通滤波

	bsp_mpu_SetRate(1);// 设置采样速率: 1000 / (1 + 0) = 1000Hz
...
void sensors_Init(void)
{
	...
	sensors_ObjectInit();				/*  创建传感器信号量以及消息队列  */
...
{	
	...
	sensors_Axis3fLpf((lpf2pData_t *)(&gyroLpf), &sensors.gyro);		/*  进行低通滤波  */
...
{
	sensors_Init();			/*  首先初始化相关传感器  */
	vTaskDelay(100);		/*  等待传感器数据稳定  */
	sensors_SetupSalveRead();	/*  将磁力计和气压计挂载到MPU上面,作为从设备  */
...
//			sensors_ProcessMagnetomterData(mag);
			...
			vTaskSuspendAll();		/*  挂起所有任务,保证传感器数据同时发送到队列  */
...
*                      sensors_GetGyroData                    
*
* Description: 单独读取陀螺仪数据
*             
* Arguments  : 陀螺仪数据缓存区地址
*
* Reutrn     : 是否读取成功
*
* Note(s)    : None.
...
*                       sensors_GetAccData                   
*
* Description: 单独读取加速度计数据
*             
* Arguments  : 加速度数据缓存区地址
*
* Reutrn     : 是否读取成功
*
* Note(s)    : None.
...
*                         sensors_GetMagData                 
*
* Description: 单独读取磁力计数据
*             
* Arguments  : 磁力计数据缓存区地址
*
* Reutrn     : 是否读取成功
*
* Note(s)    : None.
...
*                         sensors_AcquireData                 
*
* Description: 从传感器数据队列中读取传感器数据
*             
* Arguments  : 传感器数据缓存区地址
*
* Reutrn     : None.
*
* Note(s)    : None.
